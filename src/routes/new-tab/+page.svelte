...
{:then _}
    ...
    <div class="d-flex align-items-center justify-content-space-between gap-2">
        ...
        {#if $state.editable}
            <div class="save-status flash-and-hide hidden {saveStatusClass}" class:hidden="{saveStatusHidden}">
                {saveStatusMsg}
            </div>
    ...
    <div class="content">
        {#each $newTabData["data"] ?? [] as entry, idx}
            <svelte:component this="{componentMapping[entry.type].clazz}" idx="{idx}" bind:this="{shortcuts[entry.shortcut?.toLowerCase()]}" />
        {/each}
    </div>
    {#if $state.editable}
        <hr />
